...
function addThreeToArray(arr) {
  let final =[];
  for(let number of arr){
    final.push(addThree(number))
  }
  return final;
}
addThreeToArray([1,3,5,6,7]);
/*
...
function addFiveToArray(arr) {
  let final =[];
  for(let number of arr){
    final.push(addFive(number))
  }
  return final;
}
console.log(addFiveToArray([1,3,5,6,7]));
